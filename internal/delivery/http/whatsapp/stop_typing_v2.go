...
func (handler *whatsAppHandler) StopTypingV2(w http.ResponseWriter, r *http.Request) {
	...
	errUseCase := handler.whatsAppUseCase.StopTypingV2(context.Background(), chatType.GetValue(), req.Msisdn, id, req.IsAudio)
	if errUseCase != nil {
		utils.RespondWithError(w, exceptions.MapToHttpStatusCode(exceptions.ERRBUSSINESS), errUseCase.Errors.Errors)
		return
	}

	utils.RespondWithJSON(w, http.StatusOK, map[string]string{"status": "typing_stopped"})
}
